...
abstract class Category implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Lib\\Map\\CategoryTableMap';
    ...
    /**
     * Initializes internal state of Lib\Base\Category object.
     */
    ...
    public function setKey($v)
    {
        ...
        if ($this->key !== $v) {
            $this->key = $v;
            $this->modifiedColumns[CategoryTableMap::COL_KEY] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CategoryTableMap::translateFieldName('Key', TableMap::TYPE_PHPNAME, $indexType)];
            $this->key = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : CategoryTableMap::translateFieldName('CreatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->created_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : CategoryTableMap::translateFieldName('UpdatedAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Lib\\Category'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(CategoryTableMap::COL_KEY)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(CategoryTableMap::COL_KEY)) {
            $criteria->add(CategoryTableMap::COL_KEY, $this->key);
        }
        if ($this->isColumnModified(CategoryTableMap::COL_CREATED_AT)) {
     ...
     * objects.
     *
     * @param object $copyObj An object of \Lib\Category (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Lib\Category Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    public function setDayPlanRecipes(Collection $dayPlanRecipes, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($dayPlanRecipesToDelete as $dayPlanRecipeRemoved) {
            $dayPlanRecipeRemoved->setCategory(null);
        }
    ...
    protected function doAddDayPlanRecipe(ChildDayPlanRecipe $dayPlanRecipe): void
    {
        ...
        $dayPlanRecipe->setCategory($this);
    }
    ...
    public function removeDayPlanRecipe(ChildDayPlanRecipe $dayPlanRecipe)
    {
        if ($this->getDayPlanRecipes()->contains($dayPlanRecipe)) {
            ...
            $dayPlanRecipe->setCategory(null);
        }
